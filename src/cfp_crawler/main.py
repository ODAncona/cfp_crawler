...
llm = ChatCerebras(model="llama-3.3-70b", api_key=api_key_cerebras)
...
def get_total_pages(keyword: str) -> int:
    ...
    logger.info(
        f"Extraction du nombre total de pages pour le mot-clé : {keyword}"
    )
    ...
    for td in soup.find_all("td", align="center"):
        ...
        if "Total of" in text:
            ...
            if m:
                ...
                return total_pages
    logger.warning(
        "Nombre de pages non trouvé, utilisation de 1 comme valeur par défaut."
    )
    return 1
...
def search_wikicfp_page(keyword: str, page: int) -> set:
    ...
    """
    search_url = (
        f"http://www.wikicfp.com/cfp/call?conference={keyword}&page={page}"
    )
    logger.info(
        f"Recherche de CFP sur la page {page} pour le mot-clé : {keyword}"
    )
    response = requests.get(search_url)
    soup = BeautifulSoup(response.text, "html.parser")
    base_url = "http://www.wikicfp.com"
    links = soup.find_all(
        "a", href=lambda href: href and "event.showcfp" in href
    )
...
def parse_cfp_detail_page(url: str) -> CFP:
    ...
    acronym = (
        acronym_tag["content"].strip()
        if acronym_tag and acronym_tag.has_attr("content")
        else "N/A"
    )
    ...
    logger.debug(
        f"CFP extrait : Titre = {title}, Acronyme = {acronym}, When = {when}, Where = {where}, Deadline = {deadline}"
    )
    return CFP(
        title=title,
        acronym=acronym,
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Date limite (YYYY-MM-DD) en-dessous de laquelle on ignore la conférence. "
            "Par défaut, 1er janvier de l'année courante."
        ),
    )
    args = parser.parse_args()

    # Vérification de la date limite fournie
    if args.date_threshold:
        threshold_date = dateparser.parse(args.date_threshold)
        if not threshold_date:
            logger.warning(
                f"La date '{args.date_threshold}' n'a pas pu être interprétée. "
                f"On utilisera le 1er janvier de l'année courante à la place."
            )
            threshold_date = datetime(datetime.now().year, 1, 1)
    else:
        threshold_date = datetime(datetime.now().year, 1, 1)

    logger.info(
        f"Seuil de date utilisé : {threshold_date.strftime('%Y-%m-%d')}"
    )
    ...
    for page in tqdm(range(1, total_pages + 1), desc="Pages", unit="page"):
        ...
        for url in tqdm(
            detail_urls, desc=f"Page {page} - CFP", leave=False, unit="cfp"
        ):
            try:
                cfp = parse_cfp_detail_page(url)
                logger.info(f"Évaluation du CFP : {cfp.acronym}")

                # 1) On parse la date-limite si possible
                cfp_deadline_date = dateparser.parse(cfp.deadline)

                # 2) Si la date-limite n'est pas parseable, on peut décider de la considérer comme "invalide" ou "passée"
                if not cfp_deadline_date:
                    logger.warning(
                        f"Impossible de parser la deadline pour {cfp.acronym} : '{cfp.deadline}'. On l'ignore."
                    )
                    continue

                # 3) Comparaison avec la date de seuil
                if cfp_deadline_date < threshold_date:
                    logger.info(
                        f"Deadline {cfp_deadline_date.date()} < seuil {threshold_date.date()}. Conférence ignorée."
                    )
                    continue

                # 4) Sinon, on évalue la conférence
                ...
                if evaluation.score > 5:
                    ...
                    logger.info(
                        f"CFP pertinent trouvé : {cfp.acronym} (Score : {evaluation.score})"
                    )
                else:
                    logger.info(
                        f"CFP non pertinent : {cfp.acronym} (Score : {evaluation.score})"
                    )
            except Exception as e:
        ...
        if evaluated_results:
            ...
            logger.info(
                f"Résultats sauvegardés temporairement dans {args.output}"
            )
